# Qwen Instructions for 'Timothy for Change'
...
- Schema Generation: Run `npm run generate:schemas` to convert Zod schemas to JSON schemas (happens automatically in dev/build). Do not manually edit JSON schema files in `src/schemas/json/` as they are auto-generated.
